...
if [[ ! -z `sudo grep "$TG_S" ${tgdb_user}${mode_now}_node_list.txt` ]]; then
	add_contents=$(sudo grep -i "$TG_S" ${tgdb_user}${mode_now}_node_list.txt)
else
...
function do_collect() {
        ...
        sudo \cp -f /tmp/*_node_list.txt ${tgdb_user}

        for mode in DMR DSTAR NXDN P25 YSF; do
                sudo sed -i "1s/.*/Global|||======GLOBAL=======/g" ${tgdb_user}${mode}_node_list.txt;
        done
}
...
function do_pushfile() {
        mode=$1
        sudo cat ${tgdb_user}${mode}_fvrt_list.txt ${tgdb_user}${mode}_node_list.txt > /tmp/${mode}_node_list.txt;
...
if [ "$1" = "add_fvrt" ]; then
	...
	else sudo sed -i -e "/$TG_S/d" ${tgdb_user}${mode_now}_fvrt_list.txt
	fi
	line_no_file=$(sudo cat ${tgdb_user}${mode_now}_fvrt_list.txt | wc -l)
	line_no=$2
	if [ $line_no -gt $line_no_file ] || [ $line_no -eq 0 ]; then
		# add line at the bottom
		echo "$add_contents" | sudo tee -a ${tgdb_user}${mode_now}_fvrt_list.txt
	else
		# add line below $line_no
		sudo sed -i -e "${line_no}i $add_contents" ${tgdb_user}${mode_now}_fvrt_list.txt
	fi
	do_pushfile $mode_now
		...
		sudo sed -i -e "/$TG_S/d" ${tgdb_user}${mode_now}_fvrt_list.txt
		## below 2 lines are : If there are more than 2 lines matching $TG_S, get the line number of 1st matching and delete that line.
		# line_del=$(sudo grep -n "$TG_S" ${tgdb_user}${mode_now}_fvrt_list.txt | cut -d: -f1 | tail -1)
		# sudo sed -i "${line_del}d" ${tgdb_user}${mode_now}_fvrt_list.txt
		do_pushfile $mode_now
	fi
	...
	if [ -d ${tgdb_user}${LN} ]; then
        	sudo \cp -f ${tgdb_user}${LN}/* ${tgdb_user}
	else
        	sudo \cp -f ${tgdb_user}EN/* ${tgdb_user}
	fi
